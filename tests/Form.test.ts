...
describe('on component creation', () => {
  let onChangeProp: SpyInstance = vi.fn();
  // mocks automatically cleared before every test because of clearMocks global test setting
  let value: any;
  const schema = {
    type: 'object',
    title: 'root object',
  ...
  let { rerender } = render(Form, { onChange: onChangeProp, schema, value });

  function createComponent() {
    rerender({ props: { onChange: onChangeProp, schema, value } });
  }

  describe('when props.value does not equal the default values', () => {
    beforeEach(() => {
      value = {
        foo: 123
      };
...
describe('Error contextualization', () => {
  ...
  describe('on form submitted', () => {
    ...
    it('should reset errors from UI after correction and resubmission', () => {
      ...
      const errorListHTML = '<li class="text-danger">should NOT be shorter than 8 characters</li>';
...
describe('Deprecated autocomplete attribute', () => {
  ...
  it('should log deprecation warning when it is used', () => {
    ...
    expect(console.warn.calledWithMatch(/Using autocomplete property of Form is deprecated/)).to.be
      .true;
  });
...
describe('Changing the tagName', () => {
  it('should render the component using the custom tag name', () => {
    ...
    expect(container.tagName).eql('DIV');
  });
});
